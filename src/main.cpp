...
int main(int argCount, char **argList)
{
	...
	if (!outputFile)
	{
		...
		if (index == -1)
		{
			...
			outputFile = new(std::nothrow) char[length + 6];
			if (!outputFile)
			{
		...
		else
		{
			...
			outputFile = new(std::nothrow) char[outLength + 1];
			if (!outputFile)
			{
				std::cout << "Error:  Could not generate output file." << std::endl;
				return -1;
	...
	std::cout << "#Textures:\t" << header.TextureCount << std::endl << std::endl;
	...
	oMeshFile.write((char *)&header, sizeof(cmesh_header));
	...
	if (oMesh->GroupList)
	{
		for (int i = 0; i < oMesh->GroupCount; i++)
		{
			...
			std::cout << "\tMat:\t\t" << current->MaterialIndex << std::endl;
			std::cout << "\tTex:\t\t" << current->TextureIndex << std::endl;
			std::cout << "\t#Vertices:\t" << current->VertexCount << std::endl;
			std::cout << "\t#Indices:\t" << current->IndexCount << std::endl << std::endl;
